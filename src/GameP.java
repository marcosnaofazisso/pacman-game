	...
	private void run() {
		...
		while (player.getQtdeVidas() > 0) {
			...
			if (player.getPosicaoX() == booster.getPosicaoX() && player.getPosicaoY() == booster.getPosicaoY()) {
				...
				System.out.println("Est� invenc�vel!");
			}
			...
			if (desligarInvencibilidade == turno) {
				...
				booster.setVisivel(true);
				System.out.println("N�o est� mais invenc�vel");
			}
		...
		JOptionPane.showMessageDialog(this,"Parab�ns! Voc� conseguiu sobreviver por " + turno + " turnos");
